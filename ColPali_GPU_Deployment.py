...
# MAGIC %pip install colpali-engine==0.3.10 \
# MAGIC   # "git+https://github.com/illuin-tech/colpali" # from source
...
def load_colpali_model():
    """
    Load ColPali model with optimized configuration.
    ...
    model = ColPali.from_pretrained(
        model_name,
        torch_dtype=torch.bfloat16,
        device_map=current_device,  # or "mps" if on Apple Silicon
    ).eval()
...
test_scores = test_basic_functionality()
        ...
        "served_entities": [
            {
                ...
                "entity_version": "1",
                "workload_type": "GPU_MEDIUM", # comment if using cpu
                "workload_size": "Large" if torch.cuda.is_available() else "Medium",
...
# MAGIC
# MAGIC 1. **Updated colpali-engine**: Using version >=0.3.0 with improved APIs
# MAGIC 2. **score_multi_vector method**: More efficient than manual cosine similarity
# MAGIC 3. **Better device handling**: Automatic GPU/CPU detection and configuration
...
# MAGIC 6. **Flash attention support**: Faster inference with flash-attn
# MAGIC
# MAGIC ### Performance Notes
# MAGIC
# MAGIC - **GPU deployment**: ~5× throughput improvement with CUDA
# MAGIC - **Memory usage**: ~6GB for model weights + index
# MAGIC - **Index size**: ~200× more vectors than CLIP (trade-off for accuracy)
# MAGIC - **Flash attention**: Reduces memory usage and improves speed
...
# MAGIC   native method
# MAGIC - **GPU/CPU optimized**: Automatic device detection and configuration
